...
## The real recording — `realviz/`

A second, independent site: the same detector run against an actual Pulsoid export instead of the
synthetic day. Published at **`/real/`**, separate from the synthetic-day page, with its own
builder that writes only inside `docs/real/`.

```
realviz/ingest.py      pulsoid-report-*.csv  →  data/real/hr_10s.csv
realviz/synth_walk.py  a matched-length walking session  →  data/real/walk_10s.csv
realviz/detect.py      calibration, gate, scoring, both tracks
realviz/build.py       _template.html + payload  →  docs/real/
```

```
python3 realviz/ingest.py && python3 realviz/synth_walk.py && python3 -m realviz.build
```

Three things the real data forced that the synthetic day did not.

**The Pulsoid schema is not ours.** One row per beat report, every field quoted, `date_time_utc`
plus a local-time column, spacing that wanders between 3 and 7 s, and seconds omitted when they are
zero. `ingest.py` bins it onto the same 10 s lattice the detector already expects.

**The optical sensor lies, in a specific way.** Four runs in this recording sit at 1.7–2.3× the
local median and are entered faster than a heart can move — 70 → 136 bpm inside one 4-second
sample, held for a minute, then back. That is second-harmonic lock, not tachycardia. Both
signatures must fire before a sample is masked, so a genuine climb to 2× baseline survives. Left
in, the largest "stress onset" in the file would have been a glitch. The viewer draws the masked
samples rather than hiding them.

**β is measured, not inherited.** Different sensor, different rate, 18.5 % dropout, so the
synthetic day's β does not transfer. The floor here is +0.15 ± 2.38 bpm/min using a
median-absolute-deviation σ — robust to the real climbs, which a plain standard deviation would
absorb — putting β at 7.99 bpm/min, 3.3 σ.

### Why there is a synthetic walk on the page

Pulsoid streams heart rate and nothing else. With one signal there is no movement gate, and a rise
is indistinguishable from a rise because you stood up. Rather than invent a step channel for the
real recording, the real track is scored with **the gate assumed open** — every unexplained climb
charged as stress, which makes its 5 episodes an upper bound — and a synthetic walking session of
exactly the same duration is laid on the same clock to show what that assumption costs.

The synthetic walk is not hand-drawn. Cadence is; heart rate follows it through a first-order
on-kinetics model (`τ_up` 42 s, `τ_down` 105 s) plus cardiac drift, so the walking rises are
*generated by cadence*. When the detector stays silent through them, the gate is doing the work.

| synthetic walk, same β | episodes | stress load | false positives |
|---|---|---|---|
| with its step channel | 5 | 242 score·min | 1 |
| gate forced open, as the real track must be | 10 | 999 score·min | 68 |

Precision 0.909, recall 0.714, 4/4 labelled onset spans caught, 0 false positives on the negative
controls. The single false positive while walking is one sample at the first step of a bout, where
HR has begun to climb and cadence has not yet crossed α — the same class of artifact as the
crosswalk case on the synthetic day. `stress_sustained` is excluded from the numbers: a derivative
detector structurally cannot see elevated-but-flat, and counting it as a miss would be dishonest.

`realviz/build.py` re-runs the JavaScript detector's logic in numpy and fails the build unless it
agrees with `detect.py` on every sample of both tracks.

> The raw `pulsoid-report-*.csv` is gitignored. `data/real/` holds the derived 10 s grid, which is
